...
def main(neg_rate, submission_num, n_iter, train_path, test_path):
    ids = [x for x in pp.get_int_field('id', test_path)]
    clicks = pp.get_int_field('click', train_path)
    # Get Data Generators
    train = pp.data_generator(pp.clean_parse_row, train_path)
    test = pp.data_generator(pp.clean_parse_row, test_path)
    ...
    act = [x for x in pp.get_int_field('click', test_path)]

    print(ml.llfun(act, preds))
...
if __name__ == '__main__':
    ...
    usage = '''Train SGD model and create entry for Kaggle Avazu Competition.
        Usage:
        %(program_name)s --neg_rate=<r> --submission_num=<s> --n_iter=<n> --train_path=<p> --test_path=<p>
        ...
        --test_path=<p>         Path to test dataset
